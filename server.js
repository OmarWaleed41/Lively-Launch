...
const jsonPath = path.join(base_path, 'path.json');
const positionPath = path.join(base_path, 'position.json');
...
function writePath(appName, appPath, img, isAdmin) {
  ...
  fs.writeFileSync(newImgPath, img.buffer);
}
...
let edit_mode = false;
app.get('/edit-mode', (req, res) => {
  res.json({ editMode: edit_mode });
});
app.post('/edit-mode', (req, res) => {
  edit_mode = req.body.editMode;
  res.json({ success: true });
});
